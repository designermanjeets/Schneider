...
var zoomPlugin = {
        ...
        if (chartInstance.zoom._dragZoomStart) {
          ...
          if (dragDistance > 0) {
            doZoom(chartInstance, zoom, {
             	x: (startX - chartArea.left) / (1 - dragDistance / chartDistance) + chartArea.left,
  ...
  beforeDatasetsDraw: function(chartInstance) {
    ...
    var chartArea = chartInstance.chartArea;
    var dragOptions = chartInstance.options.zoom ? chartInstance.options.zoom.drag : null;
    ...
    if (chartInstance.zoom._dragZoomEnd) {
      ...
      if (dragOptions) {
        ctx.fillStyle = dragOptions.backgroundColor;
